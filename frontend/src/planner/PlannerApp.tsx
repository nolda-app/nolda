...
export default function PlannerApp() {
  ...
  // URL 경로 라우팅 — / (검색), /saved (저장한 코스), /course/:id (코스 상세).
  // 별도 라이브러리 없이 History API로 tab·openId와 주소를 양방향으로 맞춘다.
  ...
  useEffect(() => {
    const applyFromLocation = () => {
      ...
      if (courseMatch) { setTab('search'); setOpenId(courseMatch[1]) }
      else if (path === '/saved') { setTab('saved'); setOpenId(null) }
      else { setTab('search'); setOpenId(null) }
    }
  ...
  useEffect(() => {
    ...
    const path = !done ? '/' : openId ? `/course/${openId}` : tab === 'saved' ? '/saved' : '/'
